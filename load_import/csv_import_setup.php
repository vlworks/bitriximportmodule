...
if (($ACTION == 'IMPORT_EDIT' || $ACTION == 'IMPORT_COPY') && $STEP == 2)
{
    ...
    if (isset($arOldSetupVars['multi_separator_type']))
        $multi_separator_type = $arOldSetupVars['multi_separator_type'];
}
...
{
	?><tr class="heading">
		<td colspan="2"><? echo GetMessage("CATI_DATA_LOADING"); ?> (v1.0.2)</td>
	</tr>
	<tr>
...
if ($STEP == 2)
{
	...
	<tr>
		<td valign="top" width="40%">
			<div id="type_r_razdel_ttl" style="background-color: #f7e937;display: <? echo ('R' == $fields_type ? 'inline-block' : 'none'); ?>;"><? echo GetMessage("CATI_RAZDEL_TYPE"); ?>:</div>
	...
	<tr>
		...
		?><input type="hidden" name="first_names" value="N"><input type="checkbox" name="first_names" value="Y" <? if ('Y' == $first_names) echo "checked"; ?> checked></td>
	</tr>
    <tr id="multi_separator" class="heading">
        <td colspan="2"><label>Разделитель для свойств с множественным выбором</label><br></td>
    </tr>
    <tr id="multi_separator1" style="background-color: #f7e937;">
        <td class="adm-detail-valign-top">
            Разделитель<br>
            <small>Например "|", для поля со значением: "красный|синий|черный":</small>:
        </td>
        <td>
...
/*		if (!empty($arSectionProps))
	...
	<tr>
		...
		<td width="60%">
			...
			<input type="checkbox" name="IMAGE_RESIZE" id="IMAGE_RESIZE_Y" value="Y" <? echo (isset($IMAGE_RESIZE) && 'Y' == $IMAGE_RESIZE ? "checked": ""); ?> checked>
		</td>
	</tr>
	<?
	...
	if ($boolOutTranslit)
		$USE_TRANSLIT = 'N';
	?>
	<tr style="background-color: #f7e937;">
		...
		<td width="60%">
			...
			{
				?><input type="checkbox" name="USE_TRANSLIT" id="USE_TRANSLIT_Y" value="Y" <? echo (isset($USE_TRANSLIT) && 'Y' == $USE_TRANSLIT ? ' checked' : ''); ?>><small style="color: red;"> Включить для родительских и простых товаров</small><?
			}
			?>
		</td>
	</tr>
